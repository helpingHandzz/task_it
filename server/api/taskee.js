...
// Get Taskee Tasks by id
router.get("/taskee/tasks/:id", async (req, res, next) => {
  try {
    const taskeeId = parseInt(req.params.id);
    const taskeeTasksById = await prisma.task.findMany({
      where: { 
        assignedTo: taskeeId 
      },
      include: {
    ...
    res.status(200).json(taskeeTasksById)
  } catch (error) {
