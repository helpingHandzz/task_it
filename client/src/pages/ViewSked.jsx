...
import {
	deleteTaskeeScheduleThunk,
	getTaskeeScheduleThunk,
} from "../store/taskee";
import {
	format,
	parseISO,
...
export default function ViewSked() {
	...
	const taskeeId = useSelector(
		(state) => state.auth.user.taskeeId
	);
	const schedule = useSelector(
		(state) => state.taskee.workSchedule
	);
	const [isModalOpen, setIsModalOpen] = useState(false);
	const [
		selectedTaskeeSchedule,
		setSelectedTaskeeSchedule,
	] = useState(null);

	console.log("Taskee Schedule Id", selectedTaskeeSchedule);
	console.log("TaskeeId", taskeeId);
	...
	const confirmDelete = () => {
		if (selectedTaskeeSchedule) {
			dispatch(
				deleteTaskeeScheduleThunk(selectedTaskeeSchedule)
			)
				.then(() => {
					closeModal();
				})
		...
		? schedule
				.filter(
					(day) =>
						...
						isAfter(parseISO(day.date), today)
				)
				.sort((firstDate, secondDate) =>
					compareAsc(
						parseISO(firstDate.date),
						parseISO(secondDate.date)
					)
				)
		: [];
