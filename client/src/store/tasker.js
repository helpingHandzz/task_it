...
export const getTaskersThunk = () => async (dispatch) => {
    try {
        const { data: taskers } = await axios.get(`${BASE_URL}/api/taskers`);
...
export const getTaskerThunk = (id) => async (dispatch) => {
    try {
        const { data: tasker } = await axios.get(`/api/taskers/${id}`);
...
export const getTaskerReviewsThunk = (id) => async (dispatch) => {
    try {
        const { data: taskerReview } = await axios.get(`${BASE_URL}/api/taskers/reviews/${id}`)
