...
export const loginTaskeeThunk = (credentials) => async (dispatch) => {
  try {
    const { email, password } = credentials;
    const { data } = await axios
...
export const registerTaskeeThunk = (credentials) => async (dispatch) => {
  try {
    const { fName, lName, email, password, phone, city, state } = credentials;
