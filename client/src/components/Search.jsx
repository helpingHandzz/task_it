...
function Search({ categories }) {
  ...
  return (
    <div className="flex flex-col justify-center items-center gap-0">
      <h2 className="text-3xl text-center my-4">What can we help you with?</h2>
        ...
        >
          {filteredData.slice(0, 5).map((val, key) => {
            ...
            return (
              <Link
                key={val.id}
                className="dataItem w-full h-5 flex items-center justify-start py-4 hover:bg-cyan-700 font-semibold hover:text-white hover:cursor-pointer"
                to={`${location}/categories/${val.id}`}
              >
                <p className="mx-3 block">{val.categoryName}</p>
              </Link>
            );
          })}
        </div>
